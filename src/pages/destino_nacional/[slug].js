    ...
    <>
      ...
      <Container>
        <div className="col-span-6 col-start-4 grid grid-cols-6 gap-6">
          <h2 className="col-span-4 font-serif text-custom-gray text-xl">
            {destination.title}
          </h2>
          <ul className="col-span-2 flex justify-end space-x-3">
            <li>
              <a href="#this" rel="noreferrer" target="_blank" title="Facebook">
                <FontAwesomeIcon
                  className="h-5 text-soft-gray w-5"
                  icon={faFacebook}
                />
              </a>
            </li>
          ...
          <h1 className="col-span-6 text-4xl text-custom-purple">
            {destination.custom_destinations_fields.summary}
          </h1>
          <p className="col-span-6 font-serif text-custom-gray text-xs">
            Publicado em:{' '}
            {new Date(destination.date).toLocaleDateString('pt-BR')}
          </p>
          <div
            className="col-span-6 teste"
            dangerouslySetInnerHTML={{
              __html: destination.custom_destinations_fields.content,
            }}
          />
        </div>
      </Container>
